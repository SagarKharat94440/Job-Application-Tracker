...
import com.sagar.JobApplicationTracker.exception.ResourceNotFoundException;
...
import com.sagar.JobApplicationTracker.util.CacheEvictService;
import com.sagar.JobApplicationTracker.util.UrlParser;
...
public class GmailIntegrationService {
    ...
    private static final int MAX_HISTORICAL_MESSAGES = 25;
    ...
    private final String APPLICATION_NAME = "JobApplicationTracker";
    ...
    public void initiateManualSync(String email) {
        ...
        try {
            ...
            log.warn("Gmail sync paused for {} because the per-user Gmail API quota was exceeded", email);
        } catch (Exception e) {
        ...
        try {
            ...
            do {
                ListMessagesResponse response = service.users().messages().list("me")
                        .setQ(query)
                        .setMaxResults((long) MAX_HISTORICAL_MESSAGES)
                ...
                if (response.getMessages() != null) {
                    for (Message msg : response.getMessages()) {
                        ...
                        if (batchItems.size() >= MAX_HISTORICAL_MESSAGES) {
                            break;
                        }
                    }
                }
...
//                }
    ...
    private static final class GmailQuotaExceededException extends RuntimeException {
        private GmailQuotaExceededException() {
            super("Gmail API per-user quota exceeded; retry after the quota window resets");
        }
    }
    ...
    private String getOrCreateLabel(Gmail service) throws Exception {
        ...
        if (list.getLabels() != null) {
            for (Label l : list.getLabels()) {
                if ("JobApplicationTracker".equalsIgnoreCase(l.getName())) return l.getId();
            }
        }
        Label newLabel = new Label().setName("JobApplicationTracker")
        ...
        if (listResponse != null && listResponse.getFilter() != null) {
            ...
            for (Filter existingFilter : existingFilters) {
                ...
                if (existingFilter.getAction() != null && 
                    ...
                    log.info("Found outdated JobApplicationTracker filter (ID: {}). Deleting for update...", existingFilter.getId());
